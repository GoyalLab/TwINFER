...
def process_param_set(rows, label, base_config):
    ...
    k_deg_mRNA_reaction_indices = np.where(reactions_df['propensity'].str.contains("k_deg_mRNA"))[0]
    # --- Create two drifted variants: one up (scale>1), one down (scale<1) ---
    update_prop_up = make_time_scaled_update(
    update_prop, k_deg_mRNA_reaction_indices, t_start=1500, tau=15, t_offset=0
    )
    update_prop_down = make_time_scaled_update(
        update_prop, k_deg_mRNA_reaction_indices, final_value=0.12, t_start=1500, tau=15, t_offset=0
    )
    
    t_parent_end = base_config['simulation_time_before_division']

    update_prop_up_twins = make_time_scaled_update(
        update_prop, k_deg_mRNA_reaction_indices, t_start=1500, tau=15, t_offset=t_parent_end
    )
    update_prop_down_twins = make_time_scaled_update(
        update_prop, k_deg_mRNA_reaction_indices, final_value=0.12, t_start=1500, tau=15, t_offset=t_parent_end
    )
    # Quick verification
    print("\n=== CHECKING TIME-SCALED UPDATES ===")
    print(f"k_on reaction indices being modified: {k_deg_mRNA_reaction_indices}")
    print(f"Number of reactions affected: {len(k_deg_mRNA_reaction_indices)}")
    print("\nAffected reactions:")
    for idx in k_deg_mRNA_reaction_indices:
