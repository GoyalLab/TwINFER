...
output_folder = "/home/gzu5140/Keerthana_b1042/grnInference/simulation_data/pulse_three_gene_motif_longer_pulse/"
...
base_config_list = [
    ...
    "output_folder": f"{output_folder}/A_and_B/", #Path to the output folder
    ...
    "param_csv": f"{path_to_code_repo}/simulation_example_input_data/median_parameter.csv", #Path to the parameters for all genes and interaction terms
    "rows_to_use": [[0,0,0]], #Rows in the parameter's csv file for each gene. Example - [0,0] will mean use row 0 parameters for both gene 1 and 2. The length should be equal to number of genes in the system.
    "output_folder": f"{output_folder}fan_out/", #Path to the output folder
    "log_file": f"{output_folder}/log.jsonl", #Path to the log file
...
},
{
    ...
    "param_csv": f"{path_to_code_repo}/simulation_example_input_data/median_parameter.csv", #Path to the parameters for all genes and interaction terms
    "rows_to_use": [[0,0]], #Rows in the parameter's csv file for each gene. Example - [0,0] will mean use row 0 parameters for both gene 1 and 2. The length should be equal to number of genes in the system.
    "output_folder": f"{output_folder}/A_to_B/", #Path to the output folder
    "log_file": f"{output_folder}/log.jsonl", #Path to the log file
...
},
{
    ...
    "param_csv": f"{path_to_code_repo}/simulation_example_input_data/median_parameter.csv", #Path to the parameters for all genes and interaction terms
    "rows_to_use": [[0,0,0]], #Rows in the parameter's csv file for each gene. Example - [0,0] will mean use row 0 parameters for both gene 1 and 2. The length should be equal to number of genes in the system.
    "output_folder": f"{output_folder}/fan_in/", #Path to the output folder
    "log_file": f"{output_folder}/log.jsonl", #Path to the log file
    "type": "fan_in",  # Name of the network used -- will be in the filename
    "number_of_parallel_parameters": 1, #Number of parameters to be run in parallel
    "number_of_cores_per_parameter": 48, #Number of cores to be used per parameter (number_of_parallel_parameters * number_of_cores_per_parameter = number of cores in your computer)
},
{
    ...
    "param_csv": f"{path_to_code_repo}/simulation_example_input_data/median_parameter.csv", #Path to the parameters for all genes and interaction terms
    "rows_to_use": [[0,0,0]], #Rows in the parameter's csv file for each gene. Example - [0,0] will mean use row 0 parameters for both gene 1 and 2. The length should be equal to number of genes in the system.
    "output_folder": f"{output_folder}/regulated_mutual/", #Path to the output folder
    "log_file": f"{output_folder}/log.jsonl", #Path to the log file
    "type": "regulated_mutual",  # Name of the network used -- will be in the filename
    "number_of_parallel_parameters": 1, #Number of parameters to be run in parallel
    "number_of_cores_per_parameter": 48, #Number of cores to be used per parameter (number_of_parallel_parameters * number_of_cores_per_parameter = number of cores in your computer)
},
{
    ...
    "param_csv": f"{path_to_code_repo}/simulation_example_input_data/median_parameter.csv", #Path to the parameters for all genes and interaction terms
    "rows_to_use": [[0,0,0]], #Rows in the parameter's csv file for each gene. Example - [0,0] will mean use row 0 parameters for both gene 1 and 2. The length should be equal to number of genes in the system.
    "output_folder": f"{output_folder}/feed_forward/", #Path to the output folder
    "log_file": f"{output_folder}/log.jsonl", #Path to the log file
...
config_index = (args.config_index)%len(base_config_list)
print(f"🔧 Running base_config #{config_index}")
base_config = base_config_list[config_index]
#%%
...
import importlib
import gillespie_script_pulse
importlib.reload(gillespie_script_pulse)
from gillespie_script_pulse import process_param_set
#%%
